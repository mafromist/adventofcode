...
let passList = `13-17 s: ssssssssssssgsssj
7-9 p: pnlzhcppvl
5-6 z: zzbwrv
...
let passList2 = passList.split("\n")

let passListSep = [];

passList2.forEach((item, index, array) => {
        item.split("").forEach((it, ind) => {
            if(it == ":") {
                passListSep.push(item.slice(0,ind),item.slice(ind + 1, item.length))
            }})})
...
passListSep.forEach((item, index) => {
    if(index % 2 == 0) {
        mainChar = item[item.length - 1];
    
    passListSep[index].split("").forEach((item2, index2) => {
        if(item2 == "-") {
            dash = index2;
/*             console.log(dash) */
        }
    ...
    listDiff.push(passListSep[index].split("").slice(0,dash).join(""),passListSep[index].split("").slice(dash + 1,space).join(""))
    
} else if(index % 2 == 1) {
    listChar.push(passListSep[index].split("").filter(item => item == `${mainChar}`).length)
}})

/* listDiff item 1 < listChar item 1 && listChar item 1 < listDiff item 2  */
...
listChar.forEach((item, index) => {
    if(parseInt(listDiff[counter]) <= parseInt(item) && parseInt(item) <= parseInt(listDiff[counter + 1])){
